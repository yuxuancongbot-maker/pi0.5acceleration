...
### 4.1.1 `LeRobotLiberoPlusDataConfig` 键名映射

`RepackTransform` 的 dict 格式为 **`{目标键: 源键}`**。
目标键必须与 `LiberoInputs` 期望的键名一致（`observation/image` 格式）：

```python
# {dest_key: source_key}
# dest 必须匹配 LiberoInputs 期望的键名（observation/image 格式）
# source 来自 libero_plus_lerobot 数据集（observation.images.front 格式）
RepackTransform({
    "observation/image":       "observation.images.front",  # 正面相机
    "observation/wrist_image": "observation.images.wrist",  # 腕部相机
    "observation/state":       "observation.state",          # 机械臂状态
    "actions":                 "action",   # 数据集用单数 'action'，目标为复数 'actions'
    "prompt":                  "prompt",
})
# action_sequence_keys=("action",)  # lerobot 构建 delta_timestamps 时用数据集原始键名
```

与标准 LIBERO（`physical-intelligence/libero`）的对比：

| 字段 | LIBERO 源键 | LIBERO-Plus 源键 | 目标键（LiberoInputs 期望） |
|------|------------|----------------|--------------------------|
| 动作 | `actions`（复数） | `action`（单数） | `actions` |
| 正面相机 | `image` | `observation.images.front` | `observation/image` |
| 腕部相机 | `wrist_image` | `observation.images.wrist` | `observation/wrist_image` |
| 状态 | `state` | `observation.state` | `observation/state` |
...
### 8.7 `KeyError: 'observation/image'` 或 `KeyError: 'actions'` in RepackTransform

```
KeyError: 'observation/image'
  File "libero_policy.py", line 52, in __call__
      base_image = _parse_image(data["observation/image"])
```

`RepackTransform` 的 dict 格式是 **`{目标键: 源键}`**，目标键必须与下游 `LiberoInputs` 期望的键名一致（`observation/image` 格式），源键来自数据集。

LIBERO-Plus 正确写法：
```python
RepackTransform({
    "observation/image":       "observation.images.front",  # dest=LiberoInputs期望, src=数据集
    "observation/wrist_image": "observation.images.wrist",
    "observation/state":       "observation.state",
    "actions":                 "action",   # 数据集单数 → 目标复数
    "prompt":                  "prompt",
})
```

常见错误：
- 把 `{src: dest}` 写成 `{dest: src}` 方向反了 → `KeyError: 'actions'`（源不存在）
- 目标键用 `image` 而不是 `observation/image` → `KeyError: 'observation/image'`（LiberoInputs 找不到）
