...
## 十、权重上传到 Hugging Face

训练好的 checkpoint 可以通过 Hugging Face Hub 传输到其他服务器。
...
### 10.2 登录 Hugging Face

需要先在 https://huggingface.co/settings/tokens 创建 Access Token（需要 write 权限），然后登录：

```bash
uv run python3 -c "from huggingface_hub import login; login(token='你的HF_TOKEN')"
```
...
# 1. 创建 repo（已存在则跳过）
api.create_repo("你的用户名/repo名", repo_type="model", exist_ok=True)

# 2. 上传整个 checkpoint 文件夹
api.upload_folder(
    folder_path="checkpoints/pi05_libero_l1_flow/l1_flow_changed/14000",
    repo_id="你的用户名/repo名",
    repo_type="model",
    commit_message="Upload L1 Flow checkpoint step 14000",
)
```
...
snapshot_download(
    repo_id="你的用户名/repo名",
    repo_type="model",
    local_dir="./checkpoints/pi05_libero_l1_flow/l1_flow_changed/14000",
)
```

### 10.5 实际使用示例

本项目使用的上传命令（checkpoint step 14000，约 15GB）：
...
login(token='hf_xxx')  # 替换为你的 token
api = HfApi()
api.create_repo('iloveai111/pi', repo_type='model', exist_ok=True)
api.upload_folder(
    folder_path='/inspire/hdd/project/inference-chip/lijinhao-240108540148/research_yuxuancong/onestep_pi/openpi/checkpoints/pi05_libero_l1_flow/l1_flow_changed/14000',
    repo_id='iloveai111/pi',
    repo_type='model',
    commit_message='Upload L1 Flow checkpoint step 14000',
)
"
...
上传后权重地址：https://huggingface.co/iloveai111/pi
